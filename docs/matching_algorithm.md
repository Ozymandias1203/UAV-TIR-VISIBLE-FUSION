...
本文档定义本项目跨光谱匹配层的设计。该层以 TWMM 为核心，用于在去畸变后的 RGB 与热红外 TIFF 图像之间建立稳定对应关系，并输出单应性矩阵与质量指标，为热信息投影到三维点云提供几何桥梁。
...
- 热红外 TIFF 图像与 RGB 图像的尺度差异应在匹配前尽量被规范化。
- 生产链路的热红外输入必须来自 TIFF 主处理帧；JPG 预览图只能用于人工检查和诊断，不得作为生产输入。
