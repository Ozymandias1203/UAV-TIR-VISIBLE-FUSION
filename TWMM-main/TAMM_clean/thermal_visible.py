...
from image_process import center_crop, bottom_right_crop, img_read, homo_save, get_cores_point, show_map_and_img, mosaic_img, stitch_img
...
class ThermalVisble(object):
    ...
    def __init__(self,
                 ...
                 scale=None,  # 提供一个默认值
                 attention_flag=False,
                 crop_offset=(0, 0)  # 裁剪区域偏移量 (offset_x, offset_y)
                 ):
        # 检查scale是否为字典，此处可修改可见光影像放大倍数（scale_value）
        if isinstance(scale, dict):
            scale_value_visible = scale.get('visible',1.0)  # 使用.get方法提供默认值
        else:
            scale_value_visible =0.77  # 如果scale不是字典，使用默认值,1.1018效果不错

        # 检查scale是否为字典，此处可修改热红外影像放大倍数（scale_value）
        if isinstance(scale, dict):
            scale_value_thermal = scale.get('thermal',1.0)  # 使用.get方法提供默认值
        else:
            scale_value_thermal =1.0  # 如果scale不是字典，使用默认值,1.1018效果不错

        # thermal_tiff = img_read(thermal_tiff_path, scale_value)
        # thermal_rgb = img_read(thermal_rgb_path, scale_value)  # 注意这里也使用了scale_value
        # visible_rgb = img_read(visible_rgb_path, 1)

        thermal_tiff = img_read(thermal_tiff_path, scale_value_thermal)
        thermal_rgb = img_read(thermal_rgb_path, scale_value_thermal)  # 注意这里也使用了scale_value
        visible_rgb = img_read(visible_rgb_path, scale_value_visible)

        self.img_name = os.path.split(thermal_tiff_path)[1].split('.')[0]
        # 使用右下角裁剪替代居中裁剪，支持偏移调整
        self.thermal_tiff = bottom_right_crop(thermal_tiff, crop_size, crop_offset)
        self.thermal_rgb = bottom_right_crop(thermal_rgb, crop_size, crop_offset)
        self.visible_rgb = bottom_right_crop(visible_rgb, crop_size, crop_offset)
        self.visible_rgb_gray = cv2.cvtColor(self.visible_rgb, cv2.COLOR_BGR2GRAY)
        self.attention_thermal = None
    ...
    def homo_warp(self,homo):
        #将热红外可视化影像做warp
        # warp后的图像应该与可见光图像尺寸一致（因为它们在同一坐标系下）
        output_width = self.visible_rgb.shape[1]
        output_height = self.visible_rgb.shape[0]
