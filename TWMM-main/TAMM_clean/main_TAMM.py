...
def one_img_process(therma_img_path, therma_rgb_path, visible_img_path, out_img, kwargs):
    ...
    thermal_visible_c = ThermalVisble(
        ...
        thermal_rgb_path=therma_rgb_path,
        visible_rgb_path=visible_img_path,
        scale=kwargs['thermal_upsample'],  # 确保传入的是字典
        crop_size=kwargs['crop_size'],
        attention_flag=False
    )
...
if __name__=='__main__':
    ...
    thermal_path = r"D:\OneDrive\Desktop\TWMM-main\datasets\example\DJI_0076_T_ST_TIFF.tif"
    ...
    kwargs = {
    'thermal_upsample': 11,   # 不进行上采样，保持原始分辨率
    'crop_size': (256, 256),    # 裁剪图像为 256x256 的大小
    'patch_size': 32,           # 图像配准时使用 32x32 的补丁
    'search_radius': 16,        # 配准时的搜索半径为 16 像素
    'level_max': 4,              # 配准时使用最多 3 层金字塔
    'scale': {'thermal': 5.0, 'visible': 1.0}  # scale 应该是字典
    }
